...
def load_feature_engineering():
    ...
    # 2. 回退到 shared/features.py
    try:
        from shared import features as shared_ati
        apply_technical_indicators = shared_ati.apply_technical_indicators
        HAS_FEATURE_ENGINE = True
...
def init_model():
    ...
    try:
        ...
        print(f"⚠️ 模型加载失败: {e}，跳过预测功能", flush=True)
        return False
...
def main():
    ...
    skip_fin = args.skip_financials or os.environ.get('SKIP_FINANCIALS', '0') == '1'
    ...
    if args.stocks:
        ...
        print(f"🎯 只处理指定股票: {target_stocks}", flush=True)
    ...
    # 如果指定了特定股票，替换原有的股票列表
    if target_stocks:
        df_list = df_list[df_list['ts_code'].isin(target_stocks)]
        if df_list.empty:
            print("⚠️ 没有匹配的股票代码", flush=True)
            return
    ...
    if max_tickers and max_tickers > 0:
        df_list = df_list.head(max_tickers)
        print(f"✅ 获取到 {len(df_list)} 只股票，开始处理... (MAX_TICKERS={max_tickers})", flush=True)
    else:
        print(f"✅ 获取到 {len(df_list)} 只股票，开始全量处理...", flush=True)
    ...
    tickers = list(df_list['ts_code'].values)
        ...
        for batch_idx, chunk in enumerate(batches):
            ...
            try:
                ...
                for fut in as_completed(process_futures):
                    if not running:
                        print("🛑 检测到中断信号，停止处理新任务", flush=True)
                        break
