...
def apply_technical_indicators(df):
    """
    提取技术指标计算逻辑 (v25 优化版)
    - 删除低重要性特征 (stoch_signal, rsi_divergence, momentum_ma_cross)
    - 删除冗余原始指标 (SMA_*, BBM, MACD, MACDs)
    - 新增高效比率特征 (bias_sma20, bb_width_ratio)
    """
    import pandas_ta as ta
    
    # 计算基础技术指标 (部分仅用于衍生特征，不直接保留)
    ...
    cols_to_drop = [
        # 冗余原始 SMA（已用于计算衍生特征）
        'SMA_5', 'SMA_10', 'SMA_20',
        # 冗余布林带原始值（保留 BBB 宽度指标）
        'BBU_5_2.0_2.0', 'BBL_5_2.0_2.0', 'BBM_5_2.0_2.0', 'BBP_5_2.0_2.0',
        # 冗余 MACD（仅保留 MACDh 柱状图）
        'MACD_12_26_9', 'MACDs_12_26_9',
        # 低重要性特征（根据上次训练的 CSV 排名垫底）
        # 'stoch_signal', 'rsi_divergence', 'momentum_ma_cross' 已从代码中移除
    ]
...
def post_process_data(processed_dfs):
    ...
    combined_df = combined_df.sort_values(['stock_code_encoded', combined_df.index.name or 'trade_date'])
...
def run_optuna_study(combined_df, base_save_path):
    ...
    # 8. Optuna 目标函数
    def objective(trial):
        logger.info(f"【Trial {trial.number + 1}】 Starting...")
        print(f"\n{'='*60}")
        print(f"【Trial {trial.number + 1}】 开始优化试验")
        print(f"{'='*60}")
        ...
        try:
            ...
            logger.info(f"Trial {trial.number}: vol_multiplier={vol_multiplier:.4f}, offset={offset:.4f}")
        except Exception as e:
            logger.error(f"Trial {trial.number}: Error setting params: {e}")
            return 0.0
        ...
        try:
            ...
            logger.error(f"Trial {trial.number}: Unexpected error creating targets: {e}")
            return 0.0

        if train_df.empty:
            logger.warning(f"Trial {trial.number}: train_df is empty after target creation.")
            return 0.0

        class_counts_raw = train_df['target_class_3'].value_counts()

        if SHOW_VERBOSE_OUTPUT:
            print(f"\nTrial {trial.number}: 训练集目标类别分布:")
        ...
        try:
            ...
            if X.empty or len(y) == 0:
                logger.warning(f"Trial {trial.number}: X or y is empty after separation.")
                return 0.0
        except Exception as e:
            logger.error(f"Trial {trial.number}: Error separating features/target: {e}")
            return 0.0
        ...
        try:
            ...
            if len(classes) < 2:
                logger.warning(f"Trial {trial.number}: Only {len(classes)} unique classes in training data.")
                return 0.0
            ...
            logger.error(f"Trial {trial.number}: Error computing class weights: {e}")
            return 0.0
        ...
        try:
            ...
            logger.error(f"Trial {trial.number}: Error building cb params: {e}")
            return 0.0
        ...
        try:
            ...
            if len(X) < n_splits * 2:
                logger.warning(f"Trial {trial.number}: Not enough data for {n_splits} splits.")
                return 0.0
            ...
            for fold, (train_idx, val_idx) in enumerate(tscv.split(X)):
                ...
                if train_idx[-1] + gap >= val_idx[0]:
                    ...
                    if len(train_idx) < 50:  # 确保训练集足够大
                        logger.warning(f"Trial {trial.number}, Fold {fold+1}: Training set too small after gap adjustment.")
                        continue
                ...
                if X_tr.empty or X_val.empty or len(y_tr_mapped) == 0 or len(y_val_mapped) == 0:
                    logger.warning(f"Trial {trial.number}, Fold {fold+1}: Training or validation data is empty.")
                    continue
                ...
                try:
                    ...
                    logger.error(f"Trial {trial.number}, Fold {fold+1}: Error during model fit: {e}")
                    return 0.0
                ...
                try:
                    ...
                    logger.error(f"Trial {trial.number}, Fold {fold+1}: Error during prediction or evaluation: {e}")
            ...
            if not f1_scores:
                logger.warning(f"Trial {trial.number}: No F1 scores recorded across all folds.")
                return 0.0
            mean_f1 = float(np.mean(f1_scores))

        except Exception as e:
            logger.error(f"Trial {trial.number}: Unexpected error during TimeSeriesSplit or CV.", exc_info=True)
            return 0.0
        ...
        try:
            ...
            if total_samples == 0:
                logger.warning(f"Trial {trial.number}: Total samples for balance metric is 0.")
                balance_metric = 0.1
            ...
            if SHOW_VERBOSE_OUTPUT:
                ...
                print(f"Objective Value (Mean F1 * Balance): {final_score:.4f}")
                print(f"Mean Macro F1 Score (across folds): {mean_f1:.4f}")
                print(f"Balance Metric (weighted): {balance_metric:.4f}")
                print(f"Target Counts: Down={count_down}, Flat={count_flat}, Up={count_up}")
                print("---------------------------\n")
            
            # 总是输出简要进度
            elapsed = time.time() - trial_start_time
            print(f"[✓ Trial {trial.number + 1} 完成] F1={mean_f1:.4f} | Balance={balance_metric:.4f} | Score={final_score:.4f} | 耗时 {elapsed:.1f}s")

        except Exception as e:
            logger.error(f"Trial {trial.number}: Error calculating final score: {e}")
            return 0.0
    ...
    def optuna_callback(study, trial):
        ...
        if trial.number % 10 == 0:
            try:
                ...
                if SHOW_VERBOSE_OUTPUT:
                    print(f"[Callback] Trial {trial.number}: 实时图表已更新。")
            except Exception as e:
                # 绘图失败不应中断训练
                logger.warning(f"[Callback] 绘图失败: {e}")
    # --- (新增结束) ---
    ...
    if study.trials and study.best_trial:
        print(f"最佳值 (Best Value): {study.best_value:.6f}")
    ...
    if study.trials and study.best_trial and study.best_value > 0.0:
        ...
        try:
            ...
            if slice_params:
                ...
                logger.warning("Skipping slice plot: None of the default parameters found in best_params.")
    ...
    if study.best_params:
        ...
        try:
            ...
            logger.error(f"Error creating final train/test targets: {e}")
        ...
        if not train_final.empty and not test_final.empty:
            try:
                ...
                if X_train_final.empty or X_test_final.empty or len(y_train_final) == 0 or len(y_test_final) == 0:
                    logger.warning("Final train/test data is empty after preparing for post-evaluation.")
                else:
                    label_mapping = {-1: 0, 0: 1, 1: 2}
                    ...
                    try:
                        ...
                        if _GPU_AVAILABLE:
                            ...
                            if SHOW_VERBOSE_OUTPUT: print("Post-evaluation: Using GPU (gpu_ram_part=0.3).")
                        else:
                            cb_params_post['task_type'] = 'CPU'
                            if SHOW_VERBOSE_OUTPUT: print("Post-evaluation: Using CPU.")

                    except Exception as e:
                        logger.error(f"Error building post-evaluation cb params: {e}, skipping post-evaluation.")
                        cb_params_post = None
                    ...
                    if cb_params_post:
                        ...
                        try:
                            ...
                            if len(classes_final_train) < 2:
                                logger.warning("Not enough classes in final training data for class weights.")
                                sw_train_final = None
                            else:
                            ...
                            logger.error(f"Error computing final training class weights: {e}")
                            sw_train_final = None

                        try:
                            if SHOW_VERBOSE_OUTPUT: print("Fitting final model for post-evaluation...")
                            ...
                            if SHOW_VERBOSE_OUTPUT: print("Final model fitting completed.")
