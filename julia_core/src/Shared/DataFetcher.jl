...
export load_data, preprocess_data, load_parquet_files, load_arrow_files, load_data_files

"""
    load_parquet_files(data_dir::String; limit::Union{Int, Nothing}=nothing)

Load multiple parquet files from a directory into a list of DataFrames.
"""
function load_parquet_files(data_dir::String; limit::Union{Int, Nothing}=nothing)
    files = glob("*.parquet", data_dir)
    if !isnothing(limit)
...
"""
    load_arrow_files(data_dir::String; limit::Union{Int, Nothing}=nothing)

Load multiple Arrow files from a directory into a list of DataFrames.
"""
function load_arrow_files(data_dir::String; limit::Union{Int, Nothing}=nothing)
    files = glob("*.arrow", data_dir)
    if !isnothing(limit)
        files = files[1:min(limit, length(files))]
    end
...
"""
    load_data_files(data_dir::String; limit::Union{Int, Nothing}=nothing)

自动检测目录中的数据格式 (Parquet/Arrow) 并加载。
优先检测 Arrow，其次 Parquet。
"""
function load_data_files(data_dir::String; limit::Union{Int, Nothing}=nothing)
    # 优先尝试 Arrow 格式
    arrow_files = glob("*.arrow", data_dir)
    if !isempty(arrow_files)
        return load_arrow_files(data_dir; limit=limit)
    end
    
    # 其次尝试 Parquet 格式
    parquet_files = glob("*.parquet", data_dir)
    if !isempty(parquet_files)
        return load_parquet_files(data_dir; limit=limit)
    end
